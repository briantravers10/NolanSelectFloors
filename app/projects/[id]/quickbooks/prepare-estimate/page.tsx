...
export default async function PrepareEstimatePage({
  params,
  searchParams,
}: {
  ...
  if (!canManageQuickBooksDocuments(actingUser)) {
    return <EmptyState message="You don't have permission to prepare QuickBooks estimates." />;
  }
  ...
  return (
    <div className="max-w-2xl">
      <PageHeader title="Prepare QuickBooks Estimate" subtitle="Review before anything is created — nothing is sent to the customer from here." />
      ...
      <Card className="p-4">
        <DocumentReviewForm
          ...
          action={createQuickBooksDocumentAction.bind(null, id, "Estimate")}
          errorMessage={error}
        />
      </Card>
    </div>
