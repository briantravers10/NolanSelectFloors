...
import { formatCurrency, formatJobNumber, plannedAssignmentShares, plannedCostOf, projectDisplayName } from "@/lib/calculations";
                      ...
                      {dayAssignments.map((a) => {
                        ...
                        return (
                          <Link key={a.id} href={`/projects/${a.project_id}`} className="flex items-center justify-between text-sm py-1 hover:text-sky-600">
                            <span>{project && <span className="font-mono text-xs text-slate-500 mr-1">{formatJobNumber(project.job_number)}</span>}{project ? projectDisplayName(project, building?.name) : (building?.name ?? "")} · {a.role_on_job}{a.time_and_half ? " (1.5x)" : ""}</span>
